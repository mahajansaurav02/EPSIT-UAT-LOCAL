...
const UserMHPropertTypeULPIN = ({
  ...
  const searchULPIN = () => {
    if (
      ...
      console.info("ULPIN for 7/12 called !!");
    } else {
      sendRequest(
        `${URLS?.BaseURL}/EPCISAPIS/getULPINDetails`,
        "POST",
        propTypeULPIN?.ulpin,
        (res) => {
          setPropULPIN(JSON.parse(res?.ResponseData));
          const obj = JSON.parse(res?.ResponseData);
          setPropTypeULPIN({
            ...propTypeULPIN,
            khataNo: "",
            naBhu: obj?.cts_no,
            district: {
              district_code: JSON.parse(res?.ResponseData?.epcis_dist_code),
              district_name: JSON.parse(res?.ResponseData?.epcis_dist_name),
              district_english_name: "",
            },
            taluka: {
              office_code: JSON.parse(res?.ResponseData?.epcis_office_code),
              office_name: JSON.parse(res?.ResponseData?.epcis_office_name),
            },
            village: {
              village_code: JSON.parse(res?.ResponseData?.epcis_vill_code),
              village_name: JSON.parse(res?.ResponseData?.epcis_vill_name),
            },
          });
        },
        (err) => {
        ...
        <Grid container spacing={2}>
          ...
          <Grid item md={2.25}>
            ...
            <TextField
              ...
              value={propTypeULPIN?.district?.district_name}
              size="small"
            />
          </Grid>
          <Grid item md={2.25}>
            ...
            <TextField
              ...
              disabled
              className="textfieldDisabled"
              value={propTypeULPIN?.taluka?.office_name}
              size="small"
            />
          </Grid>
          <Grid item md={2.25}>
            ...
            <TextField
              ...
              disabled
              className="textfieldDisabled"
              value={propTypeULPIN?.village?.village_name}
              size="small"
            />
          </Grid>
          <Grid item md={2.25}>
